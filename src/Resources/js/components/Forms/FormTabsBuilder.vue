<template>
  <div class="nav-tabs-custom" :class="{ default : hasNoTabs }">
    <ul class="nav nav-tabs">
      <li v-for="(tab, $index) in getTabs" v-if="isTab(tab) && !tab.model || isModel(tab)" v-show="isTabVisible(tab)" :model-table="isModel(tab) && getModel(tab.model) ? getModel(tab.model).slug : ''" :class="{ active : activetab == $index, 'model-tab' : isModel(tab) }" @click="activetab = $index">
