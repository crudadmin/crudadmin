...
},{"./Sidebar/Sidebar.vue":71,"vue":64,"vue-hot-reload-api":38}],66:[function(require,module,exports){
'use strict';
...
exports.default = {
  ...
  methods: {
    ...
    sendForm: function sendForm(e, action, callback) {
      ...
      if (this.model.foreign_column != null) data[this.model.foreign_column] = this.$parent.$parent.row.id;
      ...
      this.sendForm(e, action, function (response) {
        ...
        //Update existing row
            ...
            for (var key in response.data.row) {
              ...
              if (key in _this.model.fields) _this.model.fields[key].value = response.data.row[key];
            }
          }
      });
    }
...
},{"../Forms/FormInputBuilder.vue":67,"vue":64,"vue-hot-reload-api":38}],67:[function(require,module,exports){
'use strict';
...
exports.default = {
  ...
  watch: {
    field: {
      deep: true,
      handler: function handler(newField, oldField) {
        ...
        if (newField.type == 'file') this.file_from_server = true;
      }
    }
  },
...
},{"../Partials/File.vue":70,"babel-runtime/helpers/typeof":3,"vue":64,"vue-hot-reload-api":38}],68:[function(require,module,exports){
'use strict';
...
exports.default = {
  ...
  created: function created() {
    ...
    if (!this.model) this.model = this.$parent.model;
  ...
  methods: {
    ...
    filterLanguage: function filterLanguage(array) {
      if (!array) return this.saveLanguageFilter(array);

      if (this.$root.languages.length == 0) return this.saveLanguageFilter(array);

      //If is languages table
      if (this.model.slug == 'languages') return this.saveLanguageFilter(array);

      return this.saveLanguageFilter(array.filter(function (row) {

        if (!('language_id' in row)) return true;
    ...
    dateValue: function dateValue(value, field) {
      if (!value) return;

      value = value.substr(0, 10);

      if (!('date_format' in field)) return value;
    ...
    refreshRows: function refreshRows() {
      var t = this;
      this.$root.$http.get(this.$root.requests.refresh, { model: this.model.slug, id: this.getLastRowsId }).then(function (response) {
        ...
        for (var key in response.data.fields) {
          if ('options' in this.model.fields[key]) this.model.fields[key].options = response.data.fields[key].options;
        }
  ...
  computed: {
    ...
    canAddRow: function canAddRow() {
      ...
      if (this.model.maximum <= this.getRows.length) return false;
    ...
    getRows: function getRows() {
      ...
      if (this.$parent.row == null || !('id' in this.$parent.row)) {
        ...
        if (!this.model.rows) return false;
    ...
    getLastRowsId: function getLastRowsId() {
      var lastid = 0;

      //Get last id
      for (var key in this.model.rows) {
        if (this.model.rows[key].id > lastid) lastid = this.model.rows[key].id;
      }return lastid;
    }
  },
...
},{"./FormBuilder.vue":66,"./ModelRowsBuilder.vue":69,"babel-runtime/core-js/json/stringify":1,"vue":64,"vue-hot-reload-api":38}],69:[function(require,module,exports){
'use strict';
...
exports.default = {
  ...
  ready: function ready() {
    ...
    if ('limit' in localStorage) this.limit = localStorage.limit;
  ...
  filters: {
    sortable: function sortable(array) {
      return array.sort(function (a, b) {
        //Sorting numbers
        if (this.isNumericValue(this.orderby[0])) {
          if (this.orderby[1] == 1) return b[this.orderby[0]] - a[this.orderby[0]];

          return a[this.orderby[0]] - b[this.orderby[0]];
        } else {
          if (this.orderby[1] == 1) return b[this.orderby[0]].toLowerCase().localeCompare(a[this.orderby[0]].toLowerCase(), 'sk');

          return a[this.orderby[0]].toLowerCase().localeCompare(b[this.orderby[0]].toLowerCase(), 'sk');
        }
      }.bind(this));
    },
    limit: function limit(array) {
      return array.slice(this.position * this.limit - this.limit, this.position * this.limit);
    },
    stringLimit: function stringLimit(string, length) {

      if (string.length > length) return string.substr(0, length) + '...';

      return string;
    },
    languages: function languages(array) {
      return this.$parent.buffer.rows || [];
    },
    relationship: function relationship(array) {
      var data = array.filter(function (row) {
        //If is not child model
        if (this.model.foreign_column == null) return true;
      ...
      if (remains <= 0 && this.position > 1) this.position--;

      return data;
    },
    saveRows: function saveRows(array) {
  ...
  computed: {
    columns: function columns() {
      ...
      for (var i = 0; i < this.model.columns.length; i++) {
        if (this.hidden.indexOf(this.model.columns[i]) > -1) continue;
  ...
  methods: {
    updateOrder: function updateOrder(dragged, dropped) {
      ...
      var findById = function findById(_this, id) {
        for (var key in _this.model.rows) {
          if (_this.model.rows[key].id == id) return _this.model.rows[key];
        }
      };
      ...
      this.$root.$http.get(this.$root.requests.updateOrder, { model: this.model.slug, id: dragged_id, subid: dropped_id }).then(function (response) {
    ...
    showInfo: function showInfo(row) {
      ...
      if (row.updated_at != null) data += 'Posledná zmena: <strong>' + this.$root.timeFormat(row.updated_at) + '</strong><br>';
    ...
    selectRow: function selectRow(row, data, model) {
      ...
      if (data) {
        ...
        this.$root.$http.get(this.$root.requests.show, { model: this.model.slug, id: row.id }).then(function (response) {
    ...
    fieldValue: function fieldValue(row, field) {
        ...
        if (this.model.fields[field].type == 'select') {
          if ('multiple' in this.model.fields[field] && this.model.fields[field].multiple == true && $.isArray(row[field])) {
            ...
            for (var i = 0; i < rows.length; i++) {
              if (rows[i] in this.languageOptions(this.model.fields[field].options)) values.push(this.languageOptions(this.model.fields[field].options)[rows[i]]);
            }

            return values.join(', ');
          } else {
            if (row[field] in this.languageOptions(this.model.fields[field].options)) return this.languageOptions(this.model.fields[field].options)[row[field]];
          }
        }
    ...
    isNumericValue: function isNumericValue(key) {
      ...
      if (['integer', 'decimal'].indexOf(this.model.fields[key].type) > -1) return true;
    ...
    showLimit: function showLimit(i) {
      ...
      if (this.position - offset >= i + this.maxpages / 2 || this.position <= i - this.maxpages / 2 - offset) return false;
...
},{"../Partials/File.vue":70,"babel-runtime/helpers/typeof":3,"vue":64,"vue-hot-reload-api":38}],70:[function(require,module,exports){
'use strict';
...
},{"vue":64,"vue-hot-reload-api":38}],71:[function(require,module,exports){
'use strict';
...
},{"./SidebarRow.vue":72,"vue":64,"vue-hot-reload-api":38}],72:[function(require,module,exports){
'use strict';
    ...
    computed: {
        ...
        hasChilds: function hasChilds() {
            for (var key in this.row.submenu) {
                if (this.row.submenu[key].active == true) return true;
            }return false;
        }
    }
};
...
},{"vue":64,"vue-hot-reload-api":38}],73:[function(require,module,exports){
'use strict';
...
},{"../Forms/ModelBuilder.vue":68,"vue":64,"vue-hot-reload-api":38}],74:[function(require,module,exports){
"use strict";
...
},{"vue":64,"vue-hot-reload-api":38}],75:[function(require,module,exports){
'use strict';
...
},{"./components/BaseComponent.vue":65,"./components/views/BasePageView.vue":73,"./components/views/DashboardView.vue":74,"jquery-form/jquery.form.js":35,"vue":64,"vue-drag-and-drop":37,"vue-resource":52,"vue-router":63}]},{},[75]);
