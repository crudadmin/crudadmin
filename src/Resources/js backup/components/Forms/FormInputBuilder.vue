    ...
    <div class="form-group" v-if="isFile">
      ...
      <div class="form-group file-group">
        ...
        <div v-show="isMultiple && !isMultirows && getFiles.length > 0">
          <select v-bind:id="getId + '_multipleFile'" v-bind:name="'$uploaded_'+key+'[]'" data-placeholder=" " multiple>
...
<script>
  ...
  export default {
        ...
        field : {
          ...
          handler : function(newField, oldField){
            ...
            if ( this.isSelect )
            {
              ...
              if ( ! select.next().find('input').is(':focus') )
              {
                select.chosen({disable_search_threshold: 10}).trigger("chosen:updated");
              }
            }
      ...
      computed : {
        ...
        isMultipleUpload()
        {
          return this.isMultirows || this.isMultiple;
        },
        getDateFormat()
        {
