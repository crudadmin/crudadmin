...
namespace Admin\Helpers\Localization\Extractors;
...
class VueJs extends Extractor implements ExtractorInterface, ExtractorMultiInterface
{
    ...
    protected static function getTemplateFakeJs(DOMNode $dom)
    {
        ...
        // Replace all multilines inside of {{  }}
        $content = preg_replace_callback('/\{\{(.*?)\}\}/su', function ($matches) {
            return '{{' . preg_replace('/\s+/u', ' ', $matches[1]) . '}}';
        }, $content);
